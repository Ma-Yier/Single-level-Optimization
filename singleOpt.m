function [knockOut,gene,fval,sr] = singleOpt(model,biomassID,targetID,inputID,inputMass,alpha,beta)
...
[lessMatrixLeft,lessMatrixRight,equalMatrixLeft,equalMatrixRight,objFunction] = constructMatrix(newModel); %max biomass
mInPrimalMatrix=2*size(equalMatrixLeft,1)+size(lessMatrixLeft,1);
nInPrimalMatrix=size(equalMatrixLeft,2);

% normalize inner primal opt(max)
...
[rxn,gene]=transToRev(x,newModel,nInPrimalMatrix);
